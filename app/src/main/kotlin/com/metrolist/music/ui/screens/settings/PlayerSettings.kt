...
fun PlayerSettings(
    ...
    val (automixMode, onAutomixModeChange) = rememberEnumPreference(
        AutomixModeKey,
        defaultValue = AutomixMode.SMART_AUTOMIX
    )
    val (automixBassSwap, onAutomixBassSwapChange) = rememberPreference(
        AutomixBassSwapKey,
        defaultValue = true
    )
    ...
    if (showAutomixModeDialog) {
        EnumDialog(
            onDismiss = { showAutomixModeDialog = false },
            onSelect = {
                onAutomixModeChange(it)
                onCrossfadeEnabledChange(it != AutomixMode.OFF)
                showAutomixModeDialog = false
            },
            title = stringResource(R.string.automix_mode),
            current = automixMode,
            values = AutomixMode.values().toList(),
            valueText = {
                when (it) {
        ...
        Material3SettingsGroup(
            ...
            items = buildList {
                ...
                add(Material3SettingsItem(
                    ...
                    title = { Text(stringResource(R.string.automix_mode)) },
                    description = {
                        Text(
                            when (automixMode) {
                                AutomixMode.OFF -> stringResource(R.string.automix_mode_off)
                    ...
                    onClick = { showAutomixModeDialog = true }
                ))
                if (automixMode != AutomixMode.OFF) {
                    ...
                    if (automixMode == AutomixMode.SMART_AUTOMIX) {
                        add(Material3SettingsItem(
                            icon = painterResource(R.drawable.graphic_eq),
                            title = { Text(stringResource(R.string.automix_bass_swap)) },
                            description = { Text(stringResource(R.string.automix_bass_swap_desc)) },
                            trailingContent = {
                                Switch(
                                    checked = automixBassSwap,
                                    onCheckedChange = onAutomixBassSwapChange,
                                    thumbContent = {
                                        Icon(
                                            painter = painterResource(
                                                id = if (automixBassSwap) R.drawable.check else R.drawable.close
                                            ),
                ...
                add(Material3SettingsItem(
                    ...
                    description = {
                        Text(
                            if (isTransitionActive) stringResource(R.string.audio_offload_disabled_by_crossfade)
                            else stringResource(R.string.audio_offload_description)
                        )
                    },
                    trailingContent = {
                        Switch(
                            checked = if (isTransitionActive) false else audioOffload,
                            onCheckedChange = onAudioOffloadChange,
                            enabled = !isTransitionActive,
                            thumbContent = {
                                Icon(
                                    painter = painterResource(
                                        id = if (!isTransitionActive && audioOffload) R.drawable.check else R.drawable.close
                                    ),
                                    contentDescription = null,
                                    modifier = Modifier.size(SwitchDefaults.IconSize)
                                )
                    ...
                    onClick = { if (!isTransitionActive) onAudioOffloadChange(!audioOffload) }
                ))
                add(Material3SettingsItem(
                    icon = painterResource(R.drawable.graphic_eq),
